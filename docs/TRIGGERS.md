...
| `customer-group-assignment.html` | Metzler staff manually changes a customer's group in the JTL admin panel | Use cases: B2B promotion, VIP/preferred customer, Installer/Handwerker, demotion. Per Art. 13 / 14 DSGVO, customers must be informed about meaningful changes to data processing (group assignment can affect pricing, VAT rules, communication). The *"Was sich für Sie ändert"* section has 4 Smarty-conditional variants (Wholesale / VIP / Installer / Default) — wire `{if $kundengruppe == '...'}…{/if}` around each. The Wholesale variant renders by default; the others are HTML-commented in the template for the dev team to uncomment + wire. |
