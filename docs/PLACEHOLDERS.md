...
| `{$paymentURL}` | zahlungs-erinnerung | Customer-facing payment-retry / Vorkasse-link from JTL's Zahlungs-Erinnerungsemail plugin. The default JTL template uses *"folgenden Link"* + *"Jetzt bezahlen"*, so the variable is exposed by the plugin. Verify the exact variable name in your JTL install before launch — alternatives may include `{$Bestellung->paymentURL}` or a plugin-specific name. | `https://edelstahl-tuerklingel.de/jtl.php?bezahlung=…` |
| `https://[SHOP-DOMAIN]/admin/index.php?s=bestellungen&kBestellung={$kBestellung}` | amazon-pay-info *(staff alert)* | JTL backend deep-link to the order detail. **`[SHOP-DOMAIN]` is a literal placeholder** — replace with the production shop host before launch; verify the admin path matches your install. `{$kBestellung}` is the internal numeric order ID (different from `cBestellNr`). | `https://www.edelstahl-tuerklingel.de/admin/index.php?s=bestellungen&kBestellung=98765` |
| `https://sellercentral-europe.amazon.com/` | amazon-pay-info *(staff alert)* | Amazon Seller Central root — staff search by Bestellnummer from there. Deep-link to a specific transaction would require Amazon's reference ID (not exposed by JTL plugin in known versions). | static URL |
